<?php
/*
Plugin Name: JR Countdown
Plugin URI: http://www.jakeruston.co.uk/2009/12/wordpress-plugin-jr-countdown/
Description: Allows you to insert a countdown into your wordpress blog.
Version: 1.2.8
Author: Jake Ruston
...
add_action('admin_menu', 'jr_countdown_add_pages');

// action function for above hook
function jr_countdown_add_pages() {
    add_options_page('JR Countdown', 'JR Countdown', 'administrator', 'jr_countdown', 'jr_countdown_options_page');
}
...
register_activation_hook(__FILE__,'countdown_choice');

if (!function_exists("jr_countdown_refresh")) {
function jr_countdown_refresh() {
update_option("jr_submitted_countdown", "0");
}
}

function countdown_choice () {
if (get_option("jr_countdown_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_countdown";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_countdown", "1");
wp_schedule_single_event(time()+172800, 'jr_countdown_refresh'); 
} else {
...
update_option("jr_countdown_links_choice", $content);
}
}

if (get_option("jr_countdown_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_countdown_link_personal", $content);
}
}

// jr_countdown_options_page() displays the page content for the Test Options submenu
function jr_countdown_options_page() {

    // variables for the field and option names 
    $opt_name_5 = 'mt_countdown_plugin_support';
	$opt_name_6 = 'mt_countdown_title';
	$opt_name_7 = 'mt_countdown_date';
	$opt_name_8 = 'mt_countdown_description';
	$opt_name_9 = 'mt_countdown_1';
	$opt_name_10 = 'mt_countdown_2';
	$opt_name_11 = 'mt_countdown_3';
	$opt_name_12 = 'mt_countdown_4';
	$opt_name_13 = 'mt_countdown_5';
    $hidden_field_name = 'mt_countdown_submit_hidden';
    $data_field_name_5 = 'mt_countdown_plugin_support';
	$data_field_name_6 = 'mt_countdown_title';
	$data_field_name_7 = 'mt_countdown_date';
	$data_field_name_8 = 'mt_countdown_description';
	$data_field_name_9 = 'mt_countdown_1';
	$data_field_name_10 = 'mt_countdown_2';
	$data_field_name_11 = 'mt_countdown_3';
	$data_field_name_12 = 'mt_countdown_4';
	$data_field_name_13 = 'mt_countdown_5';
...
<?php
    ...
    echo "<h2>" . __( 'JR Countdown Plugin Options', 'mt_trans_domain' ) . "</h2>";
...
<?php
    ...
    $change3 = get_option("mt_countdown_plugin_support");
...
<p><?php _e("Countdown Title:", 'mt_trans_domain' ); ?> 
<input type="text" name="<?php echo $data_field_name_6; ?>" value="<?php echo $opt_val_6; ?>" size="50">
</p><hr />

<p><?php _e("Countdown Description (%d for days, %h for hours, %m for minutes, %s for seconds):", 'mt_trans_domain' ); ?> 
<textarea name="<?php echo $data_field_name_8; ?>"><?php echo $opt_val_8; ?></textarea>
</p><hr />

<p><?php _e("Date to Countdown:", 'mt_trans_domain' ); ?> 
Day: <input type="text" name="<?php echo $data_field_name_9; ?>" value="<?php echo $opt_val_9; ?>" size="4"> Month: <input type="text" name="<?php echo $data_field_name_10; ?>" value="<?php echo $opt_val_10; ?>" size="4"> Year: <input type="text" name="<?php echo $data_field_name_11; ?>" value="<?php echo $opt_val_11; ?>" size="4"> Hour: <input type="text" name="<?php echo $data_field_name_12; ?>" value="<?php echo $opt_val_12; ?>" size="4"> Minute: <input type="text" name="<?php echo $data_field_name_13; ?>" value="<?php echo $opt_val_13; ?>" size="4">
</p><hr />
...
extract($args);
$description=get_option("mt_countdown_description");
$widget_title=get_option("mt_countdown_title");
...
$month=get_option("mt_countdown_2");
$day=get_option("mt_countdown_1");
$year=get_option("mt_countdown_3");
$second=0;
$minute=get_option("mt_countdown_5");
$hour=get_option("mt_countdown_4");
...
$years = $cdown/YSECS;
$days = ($years - floor($years))*365;
$hours = ($days - floor($days))*24;
$minutes = ($hours - floor($hours))*60;
	$cdown %= MSECS;
	$seconds = $cdown;
	
$years=$years - ($years - floor($years));
$days=$days - ($days - floor($days));
$hours=$hours - ($hours - floor($hours));
$minutes=$minutes - ($minutes - floor($minutes));
$seconds=round($seconds);

$description=str_replace("%y", $years, $description);
$description=str_replace("%d", $days, $description);
$description=str_replace("%h", $hours, $description);
$description=str_replace("%m", $minutes, $description);
$description=str_replace("%s", $seconds, $description);

echo $before_title.$widget_title.$after_title."<br />".$before_widget;
...
$supportplugin=get_option("mt_countdown_plugin_support");

if ($supportplugin=="" || $supportplugin=="Yes") {
$linkper=utf8_decode(get_option('jr_countdown_link_personal'));

if (get_option("jr_countdown_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_ads_links_choice'));
...
update_option("jr_countdown_links_choice", $new);
update_option("jr_countdown_link_newcheck", "444");
}

if (get_option("jr_submitted_countdown")=="0") {
$pname="jr_countdown";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_countdown", "1");
...
wp_schedule_single_event(time()+172800, 'jr_countdown_refresh'); 
} else if (get_option("jr_submitted_countdown")=="") {
$pname="jr_countdown";
$url=get_bloginfo('url');
$current=get_option("jr_countdown_links_choice");
...
echo "<p style='font-size:x-small'>Countdown Plugin created by ".$linkper." - ".stripslashes(get_option('jr_countdown_links_choice'))."</p>";
}
echo $after_widget;
}

function init_countdown_widget() {
register_sidebar_widget('JR Countdown', 'show_countdown');
}

add_action("plugins_loaded", "init_countdown_widget");
add_shortcode('countdown', 'show_countdown');
